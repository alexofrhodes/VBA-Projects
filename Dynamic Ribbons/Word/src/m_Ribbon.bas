...
Function RibbonIni() As String
    RibbonIni = ThisDocument.Path & "\" & getFileName(ThisDocument.Name) & "_ribbon.ini"
...
Sub Ribbon_ButtonClick(control As IRibbonControl)
    ...
    If ProcedureExists(ThisDocument.vbProject, ControlLabel) Then
        Application.Run ControlLabel
    Else
        Select Case control.id
            Case "T1_G1_B1": FollowLink ThisDocument.Path
...
Function IMAGE_FOLDER() As String
    IMAGE_FOLDER = ThisDocument.Path & "\Ribbon Images\"
End Function
...
Sub Ribbon_getImage(control As IRibbonControl, ByRef returnedVal)
    ...
    If InStr(1, ImageName, ".") > 0 Then
        ...
        strPath = ThisDocument.Path & "\Ribbon Images\"
